    ...
    return (
        <Formik
            ...
            validationSchema={Yup.object({
                ...
                email: Yup.string().matches(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i, 'Неправильный email-адрес').required('Обязательное поле!'),
                amount: Yup.number().min(5, 'Не менее 5').required('Обязательное поле!'),
                currency: Yup.string().required('Выберите валюту'),
                text: Yup.string().min(10, 'Не менее 10 символов'),
                terms: Yup.boolean().required().oneOf([true], 'Подтвердите согласие')
            })}
